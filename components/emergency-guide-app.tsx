...
function levelAccent(level: EmergencyLevel) {
  if (level === "urgent") return "border-stone-200 bg-stone-50 text-stone-950"
...
function kakaoMapSearchUrl(query: string) {
      ...
      >
        <section className="mx-auto max-w-3xl space-y-4">
            ...
            <CardContent className="space-y-3">
              <div className="flex flex-wrap gap-2">
                {selected.recommendedDepartments.map((department) => (
                  <Badge
                    key={department}
                    className="rounded-full bg-white px-3 py-1 text-sm text-indigo-950 shadow-sm hover:bg-white"
                  >
                    {department}
                  </Badge>
                ))}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Card className="rounded-[2rem] border-stone-100 bg-white/95 shadow-sm">
              <CardHeader>
                <CardTitle className="text-base text-stone-800">하지 말아야 할 행동</CardTitle>
              </CardHeader>
              <CardContent>
                <ul className="space-y-2 text-sm leading-relaxed">
                  {selected.avoidActions.map((action) => (
                    <li key={action} className="rounded-2xl bg-stone-50 px-3 py-2 text-stone-950">
                      {action}
                    </li>
                  ))}
                </ul>
          ...
          <Card className="rounded-[2rem] border-teal-100 bg-teal-50/80 shadow-sm">
            <CardHeader>
            ...
            <CardContent className="flex flex-wrap gap-2">
              {selected.pharmacyItems.map((item) => (
                <Badge
                  key={item}
                  className="rounded-full bg-white px-3 py-1 text-teal-950 shadow-sm hover:bg-white"
                >
                  {item}
                </Badge>
              ))}
  ...
  return (
    <ScreenShell title="병원 어디가?" subtitle="일상 속 작은 응급처치를 빠르게 확인해요">
      <section className="mx-auto max-w-3xl space-y-5">
        <Card className="overflow-hidden rounded-[2rem] border-slate-100 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
          <CardContent className="space-y-5 p-5">
            <div className="space-y-2 rounded-[1.5rem] bg-slate-50 p-4 ring-1 ring-slate-100">
              <p className="text-sm font-semibold text-indigo-700">간단하게 확인해요</p>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  className="min-h-14 rounded-full border-slate-200 bg-slate-50 pl-10 text-base shadow-none focus-visible:bg-white"
              ...
              <div className="grid gap-2">
                {suggestions.map(({ guide, matchedKeyword }) => (
                  <button
                    ...
                    className="min-h-11 rounded-3xl border border-slate-100 bg-white px-4 py-2 text-left text-sm shadow-sm hover:bg-indigo-50"
                  >
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
          {QUICK_SYMPTOMS.map((symptom) => (
            <button
              ...
              className={`min-h-16 rounded-[1.5rem] border px-4 py-3 text-left text-sm font-semibold shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${levelAccent(
        ...
        <p className="rounded-[1.5rem] border border-slate-100 bg-white px-4 py-3 text-xs leading-relaxed text-slate-600 shadow-sm">
          {MEDICAL_DISCLAIMER}
        </p>
      </section>
    </ScreenShell>
